...
```{r tabcoor1, echo=FALSE, message=FALSE, warning=FALSE}
library(dplyr)
library(knitr)
library(printr)
...
Unidad <- c(1:N)
eps <- 0.283
xi_P <- c(0.2875, 0.7883, 0.4089, 0.8831, 
          0.9404, 0.0455, 0.5281, 0.8924,
          0.5514, 0.4566)
xi_C <- (rank(xi_P) - eps) / N
tabla <- data.frame(Unidad = Unidad, xi_P = xi_P, xi_C = xi_C) %>% arrange(xi_P)
kable(tabla %>% arrange(Unidad), caption = "Ejemplo reducido de la conformación de números aleatorios permanentes y colocados.")
```
...
```{r tabcoor2, echo=FALSE, message=FALSE, warning=FALSE}
tabla$s_1 <- c(rep(1, 3), rep(0, 7))
tabla$s_2 <- c(rep(0, 4), rep(1, 3), rep(0, 3) )
kable(tabla %>% select(Unidad, xi_P, s_1, s_2), caption = "Ejemplo de la selección de dos muestras aleatorias simples coordinadas negativamente.")
```
...
```{r tabcoor3, echo=FALSE, message=FALSE, warning=FALSE}
tabla$N_I <- c(198, 173, 184, 179, 155, 195, 162, 159, 166, 190)
tabla$p <- round(tabla$N_I / sum(tabla$N_I ), 4)
tabla$xi_pps <- round(tabla$xi_P / (N*tabla$p), 4)
tabla <- tabla %>% arrange(xi_pps)
tabla$s_1 <- c(rep(1, 3), rep(0, 7))
tabla$s_2 <- c(rep(0, 5), rep(1, 3), rep(0, 2))
kable(tabla %>% select(Unidad, xi_P, N_I, xi_pps, s_1, s_2), caption = "Ejemplo de la selección de dos muestras secuenciales de Poisson coordinadas negativamente.")
```
...
```{r tabcoor4, echo=FALSE, message=FALSE, warning=FALSE}
tabla$pi <- 3 * tabla$p
tabla$xi_P_par <- round((tabla$xi_P * (1-tabla$pi))/(tabla$pi*(1-tabla$xi_P)), 4)
tabla <- tabla %>% arrange(xi_P_par)
tabla$s_1 <- c(rep(1, 3), rep(0, 7))
tabla$s_2 <- c(rep(0, 3), rep(1, 3), rep(0, 4))
kable(tabla %>% select(Unidad, xi_P, xi_P_par, s_1, s_2), caption = "Ejemplo de la selección de dos muestras de Paretto coordinadas negativamente.")
```
