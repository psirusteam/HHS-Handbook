...
## Expresiones para el efecto de diseño en subpoblaciones

### DEFF poblacional

Interesa calcular el efecto de diseño para la estimación de $\bar{y}_s$ definida como $\hat{\bar{y}}_s = \frac{\sum_{k \in S}w_ky_k}{\sum_{k \in S}w_k}$ donde $w_k$ corresponde al factor de expansión del individuo $k$.

El efecto de diseño se define como

$$DEFF = \frac{V_p(\hat{\bar{y}}_s)}{V_{\text{MAS}}(\hat{\bar{y}}_s)}$$
$V_{\text{MAS}}(\hat{\bar{y}}_s)$ corresponde a la varianza de $\hat{\bar{y}}_s$ bajo un muestreo aleatorio simple la cuál puede ser expresada como 

$$V_{\text{MAS}}(\hat{\bar{y}}_s) = \frac{1}{n}\left(1-\frac{n}{N}\right)S^2_{y_E} = \frac{1}{n}\left(1-\frac{n}{N}\right)\frac{1}{N-1}\sum_{k \in U}(y_k - \bar{y}_U)^2$$
sändat et al indican que dicha varianza puede ser estimada como

$$\hat{V}_{\text{MAS}}(\hat{\bar{y}}_s) = \frac{1}{n}\left(1-\frac{n}{N}\right)\hat{S}^2_{y_e}$$
donde $\hat{S}^2_{y_e} = \frac{\sum_{k \in S}w_k(y_k-\hat{\bar{y}}_s)^2}{\sum_{k \in S}w_k - 1}$

### DEFF en estratos 

Para cada estrato $h = 1,...,H$, se considera un DEFF distinto asociado a la estimación de $\bar{y}_{s_h}$ definida como $\hat{\bar{y}}_{s_h} = \frac{\sum_{k \in S_h}w_ky_k}{\sum_{k \in S_h}w_k}$-
Definimos el efecto de diseño para el estrato $h$ como 

$$DEFF_h = \frac{V_p(\hat{\bar{y}}_{s_h})}{V_{\text{MAS}}(\hat{\bar{y}}_{s_h})}$$
Para el caso de un muestreo aleatorio simple se tiene que 
$$V_{\text{MAS}}(\hat{\bar{y}}_{s_h}) = \frac{N_h^2}{n_h}\left(1-\frac{n_h}{N_h}\right)S^2_{y_{E_h}} = \frac{1}{n_h}\left(1-\frac{n_h}{N_h}\right)\frac{1}{N_h-1}\sum_{k \in U}(y_k - \bar{y}_U)^2$$
donde la varianza puede ser estimada como

$$\hat{V}_{\text{MAS}}(\hat{\bar{y}}_{s_h}) = \frac{N_h^2}{n_h}\left(1-\frac{n_h}{N_h}\right)\hat{S}^2_{y_{e_h}}$$
donde $\hat{S}^2_{y_{e_h}} = \frac{\sum_{k \in S_h}w_k(y_k-\hat{\bar{y}}_{s_h})^2}{\sum_{k \in S_h}w_k - 1}$

### DEFF en dominios

En el caso de dominios, la estimación de $\bar{y}_{s_d}$ está dada por $\hat{\bar{y}}_{s_d} = \frac{\sum_{k \in S}w_kI_{k_d}y_k}{\sum_{k \in S}w_kI_{k_d}}$ donde $I_{k_d}$ es una indicatriz que vale uno cuando el individuo $k$ se encuentra en el dominio $d$ y cero en otro caso.

Definimos el efecto de diseño para el dominio $d$ como 

$$DEFF_d = \frac{V_p(\hat{\bar{y}}_{s_d})}{V_{\text{MAS}}(\hat{\bar{y}}_{s_d})}$$
En el caso del DEFF poblacional Gutierrez indica que:

$$\hat{B} = f(\hat{t}_y,\hat{N}) = \hat{t}_y/\hat{N}$$
De aquí, tenemos que 

$$E_k = \sum_{q = 1}^Qa_qy_{qk} = \frac{y_k}{N}-\frac{t_y}{N^2} = \frac{1}{N}(y_k - B)$$
$$e_k = \frac{1}{\hat{N}}(y_k - \hat{\bar{y}}_s)$$
Note que $\sum_{k\in S} w_ke_k= 0$

De aquí, es posible calcular $\hat{V}_{\text{MAS}}(\hat{\bar{y}}_s)$ como

\begin{align*}
\hat{V}_{\text{MAS}}(\hat{\bar{y}}_s) & = \frac{N^2}{n}(1-\frac{n}{N})\hat{S}^2_{e_s} \\
& = \frac{N^2}{n}(1-\frac{n}{N})\frac{1}{N-1}\sum_{k \in S}w_k(\frac{1}{\hat{N}}(y_k - \hat{\bar{y}}_s))^2 \\
& \approx \frac{N}{n}(1-\frac{n}{N})\sum_{k \in S}w_k(y_k - \hat{\bar{y}}_s)^2/(\sum_{k \in S}w_k)^2 \\
& \approx \frac{1}{n}(1-\frac{n}{N})\sum_{k \in S}w_k(y_k - \hat{\bar{y}}_s)^2/\sum_{k \in S}w_k
\end{align*}

Este razonamiento es válido dentro de los estratos, para el caso de los dominios, debemos considerar una variable adicional:

$$\hat{B}_d = f(\hat{t}_{y_d},\hat{N}_d) = \hat{t}_{y_d}/\hat{N}_d$$
De aquí, tenemos que 

$$E_{k_d} = \frac{y_{k_d}}{N_d}-\frac{t_{y_d}}{N^2_d} = \frac{I_{k_d}}{N_d}(y_{k_d} - B_d)$$
$$e_{k_d} = \frac{I_{k_d}}{\hat{N}_d}(y_{k_d} - \hat{\bar{y}}_{s_d})$$
Por tanto, podemos escribir la varianza $\hat{V}_\text{MAS}(\hat{\bar{y}}_{s_d})$ como

\begin{align*}
\hat{V}_{\text{MAS}}(\hat{\bar{y}}_{s_d}) & = \frac{N^2}{n}(1-\frac{n}{N})\hat{S}^2_{e_{s_d}} \\
& = \frac{N^2}{n}(1-\frac{n}{N})\frac{1}{N-1}\sum_{k \in S_d}w_k e_{s_d}^2 \\
& = \frac{N^2}{n}(1-\frac{n}{N})\frac{1}{N-1}\sum_{k \in S_d}w_k\left(\frac{1}{\hat{N}_d}(y_k - \hat{\bar{y}}_{s_d})\right)^2 \\
& = \frac{N^2}{n}(1-\frac{n}{N})\sum_{k \in S_d}w_k(y_k - \hat{\bar{y}}_{s_d})^2/\hat{N}^2_d 
\end{align*}
