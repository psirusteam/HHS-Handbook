...
N <- 50000
y0 <- c(rep("Ocupado", times = 40000),
       rep("Desocupado", times = 10000))
y1 <- y0
y1[y0 == "Ocupado"] <- c(rep("Ocupado", times = 25000),
                        rep("Desocupado", times = 15000))
```
...
```{r, tabsesgo1, echo=FALSE, message=FALSE, warning=FALSE}
datas <- sample_n(data.frame(y0 = y0, y1 = y1), size = N)
kable(datas  %>% head(10), caption = "Un vistazo a la población del ejemplo. 10 primeras filas de un total de 50000.")
```
...
```{r, tabsesgo2, echo=FALSE, message=FALSE, warning=FALSE}
kable(datas %>% 
  select(y0) %>% 
  count(y0) %>%          
  mutate(prop = prop.table(n)),
  caption = "Flujos netos verdaderos en la población del ejemplo antes de la pandemia por COVID-19"
  )
```

```{r, tabsesgo3, echo=FALSE, message=FALSE, warning=FALSE}
kable(datas %>% 
  select(y1) %>% 
  count(y1) %>%          
  mutate(prop = prop.table(n)),
  caption = "Flujos netos verdaderos en la población del ejemplo en medio de la pandemia por COVID-19"
  )
```
...
```{r, tabsesgo4, echo=FALSE, message=FALSE, warning=FALSE}
kable(datas %>% 
  group_by(y0, y1) %>% 
  tally() %>%     
  spread(y1, n, fill = 0),
  caption = "Flujos brutos verdaderos del cambio en el estado laboral en la población de ejemplo."
  )
```
...
p1 <- NULL
p1[y0.sm == "Ocupado"] <- 0.7
p1[y0.sm == "Desocupado"] <- 0.1
...
La muestra telefónica es más pequeña (`r n1`) que la muestra realizada cara a cara (`r n0`). Los investigadores sospechan que los sesgos de selección no son despreciables en la muestra telefónica. Las tablas \@ref(tab:tabsesgo5) y \@ref(tab:tabsesgo6) muestran los resultados basados en las muestras (no ponderados) para la encuesta cara a cara y la encuesta telefónica, respectivamente.

```{r, tabsesgo5, echo=FALSE, message=FALSE, warning=FALSE}
...
kable(Tf2f,
      caption = "Resultados observados en la muestra presencial del ejemplo.")
```

```{r, tabsesgo6, echo=FALSE, message=FALSE, warning=FALSE}
...
kable(Ttele,
      caption = "Resultados observados en la muestra telefónica del ejemplo.")
```
...
Como se ve en la tabla \@ref(tab:tabsesgo7), de los `r n1` encuestados en la encuesta telefónica, `r Ty0rsp[1,3]*100`% estaban empleados en el período anterior, y aproximadamente `r Ty0rsp[2,3]*100 `% estaban desempleados, lo que podría ser indicio de un patrón de ausencia de respuesta MAR. 

```{r, tabsesgo7, echo=FALSE, message=FALSE, warning=FALSE}
kable(Ty0rsp, caption = "Proporción observada del estado de ocupación en la muestra telefónica del ejemplo. ")
```
...
```{r, tabsesgo8, echo=FALSE, message=FALSE, warning=FALSE}
kable(Ty0non, caption = "Proporción observada del estado de ocupación en la muestra telefónica del ejemplo.")
```
...
```{r, tabsesgo9, echo=FALSE, message=FALSE, warning=FALSE}
library(rcompanion)

cross <- table(y0.sm, s1)
kable(
  as.data.frame(cross) %>% 
    rename(Estado = y0.sm, Respuesta = s1), 
      caption = "Asociación entre la respuesta telefónica y el estado de ocupación en el periodo anterior en la muestra del ejemplo.")
```
...
```{r, figsesgo1, message=FALSE, warning=FALSE, error=FALSE, fig.showtext=TRUE, echo=FALSE, fig.cap="Histograma de los *propensity-score*"}
hist(ps, main = "", xlab = "", ylab = "")
```
...
```{r, figsesgo2, message=FALSE, warning=FALSE, error=FALSE, fig.showtext=TRUE, echo=FALSE, fig.cap="Histograma de los pesos ajustados"}
hist(1/(g1k*g0k), main = "", xlab = "", ylab = "")
```
