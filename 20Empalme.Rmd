...
```{r fig201, fig.cap= "Series de tiempo para el rediseño de una encuesta. La línea negra representa la serie regular; la línea azul representa la serie nueva. Fuente: elaboración propia.",  echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}
set.seed(124567)
n <- 200
x.r <- arima.sim(n = n, list(order = c(1,1,0), ar = 0.7))/120
x.r <- ts(x.r[-1])
t0 <- 120
x.n <- x.r
x.n[t0:n] <- x.r[t0:n] + 0.3
x <- data.frame(t=1:n, x.r, x.n)
library(ggplot2)
library(dplyr)

ggplot(x) + 
  geom_line(aes(x=t, y=x.n), col =  "blue") + 
  geom_line(aes(x=t, y=x.r), col = "black")  +
  xlab("Tiempo") + ylab("Estimación") +
  theme_minimal()
```
...
```{r, fig202, fig.cap= "Empalme de series de tiempo con el método del factor suavizado. La línea negra corresponde a la serie regular; la línea azul representa la serie nueva; la línea roja denota la serie empalmada. Fuente: elaboración propia.",  echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}

factor <- x.n[t0]/x.n[t0-1] - 1
x.nadale <- x.n
x.nadale[1:(t0-1)] <- x.n[1:(t0-1)] * (1 + factor*(1:(t0-1))/(t0-1))
x$x.nadale <- x.nadale

ggplot(x) + 
  geom_line(aes(x=t, y=x.n)) +
  geom_line(aes(x=t, y=x.nadale), col = "red") + 
  geom_line(data = x %>% filter(t >= t0), aes(x=t, y=x.nadale), col = "blue") +
  xlab("Tiempo") + ylab("Estimación") +
  theme_minimal()
```
...
```{r, fig203, fig.cap= "Empalme de series de tiempo con el ajuste sintético multiplicativo. La línea negra corresponde a la serie regular; la línea azul representa la serie nueva; la línea roja denota la serie empalmada. Fuente: elaboración propia.",  echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}

n <- 120
delta <- x.n[n] - x.r[n]
x.n.adj1 <- x.n
x.n.adj1[1:(n-1)] <- x.r[1:(n-1)] + delta
x$x.n.adj1 <- x.n.adj1

f <- x.n[n] / x.r[n]
x.n.adj2 <- x.n
x.n.adj2[1:(n-1)] <- x.r[1:(n-1)] * f
x$x.n.adj2 <- x.n.adj2

ggplot(x) + 
   geom_line(aes(x=t, y=x.n.adj2), col = "red") +
   geom_line(aes(x=t, y=x.n), col = "blue") + 
    geom_line(aes(x=t, y=x.r)) + 
  xlab("Tiempo") + ylab("Estimación") +
  theme_minimal()
```
...
## y[t] = F[t]alpha[t] + v[t], v[t] ~  N(0, Vt)
## Transition equation:
## alpha[t+1] = G[t]alpha[t] + w[t], w[t] ~ N(0, Wt)
y <- x$x.n
m <- 3 # dimensión de alpha_t
Ft <- matrix(c(1,0,0), nrow=1)
jFt <- matrix(c(0,0,1), nrow=1) # valor de 0 indica que el valor de la matriz F es constante
delta <- c(rep(0, t0-1), rep(1, n-t0+1))
X <- matrix(c(rep(0, t0-1), rep(1, n-t0+1)), ncol = 1) # delta_t
Gt <- matrix(c(1,1,0,0,1,0,0,0,1), nrow = m, byrow = T)
# Especificar el modelo
modelo <- dlm(FF=Ft, GG=Gt, JFF=jFt, X=X, m0=rep(0,m), C0=1e8*diag(m), V=var(y), W=diag(c(10,10,0)))
# Estimar las varianzas desconocidas
n.sim <- 1000
qqq<-dlmGibbsDIG(y, mod=modelo, a.y=1, b.y=1000, a.theta=1,
b.theta=100, n.sample=n.sim)
# Especificar el modelo de nuevo con las varianzas estimadas
modelo2<-dlm(FF=Ft, GG=Gt, JFF=jFt, X=X, m0=rep(0,m), C0=1e8*diag(m),
V=median(qqq$dV[(n.sim*0.9):n.sim]),
W=diag(c(mean(qqq$dW[(n.sim*0.9):n.sim, 1]),mean(qqq$dW[(n.sim*0.9):n.sim, 2]), 0)))

filtro <- dlmFilter(y, modelo2)
filtro.df <- dropFirst(filtro$m)
colnames(filtro.df) <- c("nivel", "pendiente", "beta")
suavizado<-dlmSmooth(y, modelo2)
suavizado.df<-as.data.frame(dropFirst(suavizado$s))
colnames(suavizado.df) <- c("nivel", "pendiente", "beta")

suavizado.df$y <- y
suavizado.df$t <- 1:n
suavizado.df$y.adj <- suavizado.df$y
suavizado.df$y.adj[1:(t0-1)] <- suavizado.df$y[1:(t0-1)] + suavizado.df$beta[1:(t0-1)]
suavizado.df$y.adj2 <- suavizado.df$y
suavizado.df$y.adj2[1:(t0-1)] <- suavizado.df$y[1:(t0-1)] + suavizado.df$beta[1:(t0-1)] *(1:(t0-1))/(t0-1)
```
...
```{r, fig204, fig.cap= "Empalme de series de tiempo usando un modelo estructural. La línea negra corresponde a la serie regular; la línea azul representa la serie nueva; la línea roja denota la serie empalmada. Fuente: elaboración propia.",  echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}
ggplot(suavizado.df) + 
  geom_line(aes(x=t, y=y)) + 
  geom_line(aes(x=t, y=y.adj2), col = "red")  + 
  geom_line(data = suavizado.df%>%filter(t>=t0), aes(x=t, y=y), col = "blue") +
    xlab("Tiempo") + ylab("Estimación") +
  theme_minimal()
```
...
y <- x[,2:3]
m <- 3 # dimensión de alpha_t
Ft <- matrix(c(1,0,0,1,0,0), nrow=2, byrow = T)
jFt <- matrix(c(0,0,0,0,0,1), nrow=2, byrow = T) # valor de 0 indica que el valor correspondiente de la matriz F es constante
delta <- c(rep(0, t0-1), rep(1, n-t0+1))
X <- matrix(delta, ncol = 1) # delta_t
Gt <- matrix(c(1,1,0,0,1,0,0,0,1), nrow = m, byrow = T)
# Especificar el modelo
modelo <- dlm(FF=Ft, GG=Gt, JFF=jFt, X=X, m0=rep(0,m), C0=1e8*diag(m), V=var(y), W=diag(c(var(y[,1]), var(y[,1]), 0)))

filtro <- dlmFilter(y, modelo)
filtro.df <- dropFirst(filtro$m)
colnames(filtro.df) <- c("nivel", "pendiente", "beta")
suavizado<-dlmSmooth(y, modelo)
suavizado.df<-as.data.frame(dropFirst(suavizado$s))
colnames(suavizado.df) <- c("nivel", "pendiente", "beta")

suavizado.df$y.r <- y[,1]
suavizado.df$y.n <- y[,2]
suavizado.df$t <- 1:n
suavizado.df$y.r.adj <- suavizado.df$y.n
suavizado.df$y.r.adj[1:(t0-1)] <- suavizado.df$y.r[1:(t0-1)] + suavizado.df$beta[1:(t0-1)] 
suavizado.df$y.r.adj2 <- suavizado.df$y.n
suavizado.df$y.r.adj2[1:(t0-1)] <- suavizado.df$y.r[1:(t0-1)] + suavizado.df$beta[1:(t0-1)] *(1:(t0-1))/(t0-1)
```
...
```{r, fig205, fig.cap= "Empalme de series de tiempo usando un modelo estructural. La línea negra corresponde a la serie regular; la línea azul representa la serie nueva; la línea roja denota la serie empalmada. Fuente: elaboración propia.",  echo=FALSE, message=FALSE, warning=FALSE, error=FALSE}
ggplot(suavizado.df) + 
    geom_line(aes(x=t, y=y.n), col = "blue") + 
  geom_line(aes(x=t, y=y.r)) +
   geom_line(aes(x=t, y=y.r.adj2), col = "red")  + 
  geom_line(data = suavizado.df%>%filter(t>=t0), aes(x=t, y=y.n), col = "blue") + 
    xlab("Tiempo") + ylab("Estimación") +
  theme_minimal()
```
